<!-- Affichage - Les avantages de Sailing loc -->

<div class="container mt-5">
    <h3 class="text-center fs-1">Pourquoi <span class="bleu2">Sailing Loc </span> !</h3>
    <div class="row g-4 py-5 row-cols-1 row-cols-lg-3">
        <div class="col align-items-start text-center">
            <div
                class="icon-square bg-light d-inline-flex align-items-center justify-content-center fs-4 flex-shrink-0 me-3 p-2 rounded-3">
                </i><i class="fas fa-euro-sign display-3 mb-3"></i>
            </div>
            <div>
                <h3 class="fs-3 mb-2">Prix imbattables</h3>
                <p>Un large choix de bateaux à louer à tout moment. Choississez parmi nos milliers bateaux et naviguez
                    maintenant !</p>
            </div>
        </div>
        <div class="col align-items-start text-center">
            <div
                class="icon-square bg-lightd-inline-flex align-items-center justify-content-center fs-4 flex-shrink-0 me-3 p-2 rounded-3">
                <i class="fas fa-lock display-3 mb-3"></i>
            </div>
            <div>
                <h3 class="fs-3 mb-2">Sécurité et confiance</h3>
                <p>Le service de location est 100%sûr et vous permet de louer ou faire louer votre bateau en toute
                    sécurité et simplicité.</p>
            </div>
        </div>
        <div class="col align-items-start text-center">
            <div
                class="icon-square bg-light d-inline-flex align-items-center justify-content-center fs-4 flex-shrink-0 me-3 p-2 rounded-3">
                <i class="fas fa-question display-3 mb-3"></i>
            </div>
            <div>
                <h3 class="fs-3 mb-2">Support disponible</h3>
                <p>Nous sommmes disponibles 7/7 pour vous répondre depuis notre ligne, notre mail, notre chat ou notre
                    formulaire de contact. </p>
            </div>
        </div>
    </div>
</div>

<div class="container px-4 py-5" id="custom-cards">
    <h2 class="pb-2 border-bottom text center jaune">Notre trio parfait</h2>
    <div class="row row-cols-1 row-cols-lg-3 align-items-stretch g-4 py-5">
        <div class="col">
            <div class="card card-cover h-100 overflow-hidden text-bg-white rounded-4 shadow-sm"
                style="background-image: url('unsplash-photo-1.jpg');">
                <div class="d-flex flex-column h-100 p-5 pb-3 text-white text-shadow-1">
                    <h3 class="mt-5 mb-4 display-5 text-dark text-center">Louer</h3>
                    <a href="http://salingloc.local/nos-bateaux/"  class="btn btn-warning btn-lg text-dark mt-4 "
                        title="Louer un bateau maintenant ">Réserver</a>
                </div>
            </div>
        </div>

        <div class="col">
            <div class="card card-cover h-100 overflow-hidden text-bg-white rounded-4 shadow-sm"
                style="background-image: url('unsplash-photo-2.jpg');">
                <div class="d-flex flex-column h-100 p-5 pb-3 text-white text-shadow-1">
                    <h3 class="mt-5 mb-4 display-5  text-dark text-center">Permis</h3>
                    <a href="http://salingloc.local/permis-bateaux/"  class="btn btn-warning btn-lg text-dark mt-4 "
                        title="Passer un permis bateau">Commencer</a>

                </div>
            </div>
        </div>

        <div class="col">
            <div class="card card-cover h-100 overflow-hidden text-bg-white  rounded-4 shadow-sm pb-5"
                style="background-image: url('unsplash-photo-3.jpg');">
                <div class="d-flex flex-column h-100 p-5 pb-3  text-shadow-1">
                    <h3 class="mt-5 mb-4 display-5 text-dark text-center">Propriétaire</h3>
                    <a href="http://salingloc.local/vendor-register/"  class="btn  btn-lg mt-4"
                        title="Ajouter un bateau en ligne ">Ajouter un bateau</a>
                </div>
            </div>
        </div>
    </div>
</div>